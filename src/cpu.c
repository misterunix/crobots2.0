...
/*  long pop(); */
  ...
  switch(cur_instr->ins_type) {
	...
	{
		push(*(cur_robot->external + (short int)(cur_instr->u.var1 & ~EXTERNAL)));
	}	
      else
	push(*(cur_robot->local + (short int)cur_instr->u.var1));
      cur_robot->ip++;
      break;
    ...
    case STORE:		/* store tos in a variable pool */
      ...
      if (cur_instr->u.a.var2 & EXTERNAL) * (cur_robot->external + (short int)(cur_instr->u.a.var2 & ~EXTERNAL)) = push(pop());
      else
	*(cur_robot->local + (short int)cur_instr->u.var1) = push(pop());
      cur_robot->ip++;
      break;
